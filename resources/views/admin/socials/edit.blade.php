...
@section('page-title', 'Ubah Media Sosial')
@section('page-subtitle', 'Perbarui informasi akun media sosial')
...
<div class="mt-2 max-w-2xl">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-8">
        <form action="{{ route('admin.socials.update', $social) }}" method="POST" class="space-y-6">
            @csrf @method('PUT')
            <div>
                <label class="block text-sm font-semibold text-slate-700 mb-2">Nama Media Sosial *</label>
                <input type="text" name="name" value="{{ old('name', $social->name) }}" required
            ...
            <div>
                ...
                <input type="url" name="url" value="{{ old('url', $social->url) }}" required
            ...
            <div class="grid grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" name="order" value="{{ old('order', $social->order) }}"
                ...
                <div class="flex items-end pb-3">
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" {{ $social->is_active ? 'checked' : '' }} class="w-5 h-5 text-red-600 border-slate-200 rounded focus:ring-red-500">
